...
package com.yollock.kobe.util.concurrent;
...
public class ConcurrentHashMapV8<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
     ...
     * The concurrent is backed by the map, so changes to the map are
     * reflected in the concurrent, and vice-versa.  The concurrent
    ...
    final Node<K, V>[] helpTransfer(Node<K, V>[] tab, Node<K, V> f) {
        ...
        if (tab != null && (f instanceof ForwardingNode) &&
            ...
            while (nextTab == nextTable && table == tab &&
                    (sc = sizeCtl) < 0) {
                if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
                        sc == rs + MAX_RESIZERS || transferIndex <= 0) break;
        ...
        final Node<K, V> advance() {
            ...
            for (; ; ) {
                ...
                if (baseIndex >= baseLimit || (t = tab) == null ||
                        (n = t.length) <= (i = index) || i < 0) return next = null;
    ...
    static final class EntrySpliterator<K, V> extends Traverser<K, V> implements ConcurrentHashMapSpliterator<Entry<K, V>> {
    ...
    abstract static class CollectionView<K, V, E> implements Collection<E>, Serializable {
        ...
        /**
         * Returns a string representation of this concurrent.
         * The string representation consists of the string representations
         * of the concurrent's elements in the order they are returned by
         * its iterator, enclosed in square brackets ({@code "[]"}).
         * Adjacent elements are separated by the characters {@code ", "}
         * (comma and space).  Elements are converted to strings as by
         * {@link String#valueOf(Object)}.
         *
         * @return a string representation of this concurrent
         */
        public final String toString() {
            StringBuilder sb = new StringBuilder();
            sb.append('[');
    ...
    public static class KeySetView<K, V> extends CollectionView<K, V, K> implements Set<K>, Serializable {
        ...
        /**
         * Adds all of the elements in the specified concurrent to this set,
         * as if by calling {@link #add} on each one.
         *
         * @param c the elements to be inserted into this set
         * @return {@code true} if this set changed as a result of the call
         * @throws NullPointerException          if the concurrent or any of its
    ...
    static final class ForEachTransformedValueTask<K, V, U> extends BulkTask<K, V, Void> {
        ...
        public final void compute() {
            ...
            if ((transformer = this.transformer) != null && (action = this.action) != null) {
                ...
                for (Node<K, V> p; (p = advance()) != null; ) {
                    U u;
                    if ((u = transformer.apply(p.val)) != null) action.apply(u);
                }
                propagateCompletion();
            }
        }
    ...
    static final class ForEachTransformedEntryTask<K, V, U> extends BulkTask<K, V, Void> {
        ...
        ForEachTransformedEntryTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, Fun<Entry<K, V>, ? extends U> transformer, Action<? super U> action) {
            super(p, b, i, f, t);
            this.transformer = transformer;
            this.action = action;
        }
    ...
    static final class ForEachTransformedMappingTask<K, V, U> extends BulkTask<K, V, Void> {
        ...
        public final void compute() {
            ...
            if ((transformer = this.transformer) != null && (action = this.action) != null) {
                ...
                for (Node<K, V> p; (p = advance()) != null; ) {
                    U u;
                    if ((u = transformer.apply(p.key, p.val)) != null) action.apply(u);
                }
                propagateCompletion();
            }
        }
    ...
    static final class SearchEntriesTask<K, V, U> extends BulkTask<K, V, U> {
        ...
        SearchEntriesTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, Fun<Entry<K, V>, ? extends U> searchFunction, AtomicReference<U> result) {
            super(p, b, i, f, t);
            this.searchFunction = searchFunction;
            this.result = result;
        }
    ...
    static final class ReduceEntriesTask<K, V> extends BulkTask<K, V, Entry<K, V>> {
        ...
        ReduceEntriesTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, ReduceEntriesTask<K, V> nextRight, BiFun<Entry<K, V>, Entry<K, V>, ? extends Entry<K, V>> reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.reducer = reducer;
        }
    ...
    static final class MapReduceEntriesTask<K, V, U> extends BulkTask<K, V, U> {
        ...
        MapReduceEntriesTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, MapReduceEntriesTask<K, V, U> nextRight, Fun<Entry<K, V>, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
            this.reducer = reducer;
    ...
    static final class MapReduceEntriesToDoubleTask<K, V> extends BulkTask<K, V, Double> {
        ...
        MapReduceEntriesToDoubleTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, MapReduceEntriesToDoubleTask<K, V> nextRight, ObjectToDouble<Entry<K, V>> transformer, double basis, DoubleByDoubleToDouble reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
            this.basis = basis;
    ...
    static final class MapReduceEntriesToLongTask<K, V> extends BulkTask<K, V, Long> {
        ...
        MapReduceEntriesToLongTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, MapReduceEntriesToLongTask<K, V> nextRight, ObjectToLong<Entry<K, V>> transformer, long basis, LongByLongToLong reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
            this.basis = basis;
    ...
    static final class MapReduceEntriesToIntTask<K, V> extends BulkTask<K, V, Integer> {
        ...
        MapReduceEntriesToIntTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t, MapReduceEntriesToIntTask<K, V> nextRight, ObjectToInt<Entry<K, V>> transformer, int basis, IntByIntToInt reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
            this.basis = basis;
